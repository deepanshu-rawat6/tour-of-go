...
```mermaid
graph TD
    C[Client] -->|POST /shorten<br>GET /:code| RL[Rate Limiter<br>04-rate-limiter<br>token bucket 10 req/s]
    RL --> H[HTTP Handler]
    H -->|Set/Get code→url| CACHE[07-distributed-cache<br>RESP server :6380]
    H -->|PUB url.created<br>PUB url.clicked| MQ[06-message-queue<br>TCP server :9001]
    SCHED[09-task-scheduler<br>* * * * *] -->|analytics flush| LOG[log.Println]
...
for i in $(seq 12); do curl -s -o /dev/null -w "%{http_code}<br>" localhost:8087/health; done
```
